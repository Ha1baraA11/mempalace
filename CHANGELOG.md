...
- **`list_drawers` and `graph_stats` walk a Qdrant collection once.** `BaseCollection.get_all_rows()` joins `get_all_metadata()` as a single-pass contract; Qdrant overrides it with one scroll, where the offset loop had cost 275 s and 292 s on a 126k-drawer palace. (#2452, #2474)
- **Hallway recompute reads only the mined wing.** `compute_hallways_for_wing` fetched every drawer in the palace on each mine; a bounded, wing-scoped page walk keeps the #1619 variable-limit protection and turns a multi-minute CPU spike on large palaces into O(wing). (#2466, #2477)
- **CLI `status` uses the single-pass metadata read.** 1 m 46 s down to 3.4 s on a 160k-drawer Qdrant palace. (#2153, #2154)
...
- **Every JSON-RPC request that carries an `id` gets an answer.** A non-string `method` or non-object `params` raised inside dispatch, so stdio wrote nothing and HTTP closed the socket, leaving the client waiting forever. Malformed envelopes now return `-32601` / `-32602`, and a dispatch failure returns `-32603`. (#2231, #2232)
...
- **A stranded HNSW index is no longer reported as flush-lag.** When `index_metadata.pickle` is absent, an unreachable `hnsw:sync_threshold` and a replacement stub whose payload cannot hold the SQLite count are now reported as such, with the fix to apply, instead of leaving vector search enabled. (#2435)
...
- **Chroma systems are stopped before the shared cache is cleared.** On an external inode or mtime change the backend cleared Chroma's process-global system cache before closing its clients, so the displaced `System.stop()` never ran. (#2421)
